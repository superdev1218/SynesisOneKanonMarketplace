...
const AddClaimModal = ({ open, onClose, setClaimFlag, setAirdropedList }) => {
        ...
        if (nfts.length > firstIndex) {
          ...
          if (firstIndex + AirdropPgSize >= nfts.length) {
            ...
            interval = setInterval(async () => {
              ...
              getthumbnailbymintaddresses(
                state.seasonNumber,
                firstIndex + showCount * AirdropPgSize,
                AirdropPgSize,
                mintaddress
              ).then((resdata) => {
                resdata.map((item) => {
  ...
  const addClaim = async (event) => {
    ...
    dispatch(setLoading(true));
    try {
      let proof = await getreservednftproofbymintaddressandwallet(
        wallet.publicKey.toBase58(),
        nfttokens[current].nft_mint_address
      );
      const tx = await KNProgram.claimAirdrop(
        proof[0].proof.map((x) => Buffer.from(x.data)),
        nfttokens[current].nft_mint_address
      );
          ...
          <Box className={classes.itemList1}>
            <Box className={classes.itemList}>
              {nfttokens.map((val, index) => (
                <Button
                  className={classes.imageCardBtn}
                  key={index}
                  onClick={() => {
                    setCurrent(index);
                  }}
                >
                  <LazyLoadImage
                    ...
                    effect="opacity"
                    src={`data:image/png;base64,${val.thumbnail}`}
                    alt="collection"
                    className={clsx(classes.imageCard, index == current && classes.imageCardActive)}
                  />
                </Button>
              ))}
              {loadings && (
            ...
            {nfttokens[current].metadata != null ? (
              <>
                <LazyLoadImage
                  effect="opacity"
                  placeholderSrc={LoaderImage}
                  src={nfttokens[current].metadata.image}
                  alt="collection"
                ...
                <Box className={classes.content}>
                  ...
                  <Accordion className={classes.accordion} defaultExpanded={false}>
                    ...
                    >
                      <Typography className={classes.accordionTitle}>
                        {nfttokens[current].metadata.attributes.length} Attributes{' '}
                      </Typography>
                    </AccordionSummary>
                    <AccordionDetails className={classes.accordionDetails}>
                      <Box className={classes.attribute}>
                        {nfttokens[current].metadata.attributes.map((val, index) => (
                          <Box key={index} className={classes.attributeDiv}>
                  ...
                  <Box className={classes.buyDiv}>
                    <KanonColorButton className={classes.buyButton} onClick={addClaim}>
                      Claim
                    </KanonColorButton>
                  </Box>
                </Box>
