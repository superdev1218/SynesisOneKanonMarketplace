...
const Mint = () => {
  ...
  useEffect(async () => {
    ...
    if (wallet && wallet.connected && !wallet.disconnecting) {
      try {
        collectionState = await KNProgram.getProgram().account.collectionAccount.fetch(
      ...
      collectionState = await programs
        .getProgram()
    ...
    if (step == 1) {
      let txt = EstTime(
        collectionState.seasonOpenedTimestamp.toNumber() + collectionState.countdownDuration.toNumber() * 3600
      );
  ...
  const handleMint = async (event) => {
    ...
    try {
      let nfts;
      let step = 0;
      let collectionState = await KNProgram.getProgram().account.collectionAccount.fetch(
        KNProgram._collection_state_account_pubkey
      );
      if (collectionState.seasonOpenedTimestamp.toNumber() == 0) {
        step = 0;
      ...
      for (let i = 0; i < nfts.length; i++) {
        ...
        if (index != -1) {
          try {
            let tx = null;
            if (step == 2) {
              tx = await KNProgram.mintWhiteListNftOne(
                whitelist[0].proof.map((x) => Buffer.from(x.data)),
                nfts[lanIndex].mint.toBase58()
              );
            }
            if (step == 3) {
  ...
  const ImageSlide1 = () => (
    <Box>
      {imageList1.map((item, index) => (
        <LazyLoadImage effect="opacity" src={item} key={index} alt="slide" className={classes.imageItem} />
      ))}
    </Box>
  );
  const ImageSlide2 = () => (
  ...
  const BelowButton = () => {
    if (steps === 2 || steps === 3) {
      return (
        <Button className={classes.connectButton} onClick={handleMint}>
          Mint
        </Button>
      );
    }
  ...
  return (
    <Box className={classes.faqPage}>
      ...
      <Box className={classes.faqHeader}>
        <Box sx={{ display: isXs ? 'none' : 'flex' }} className={classes.header1}>
          <ImageSlide1 />
        </Box>
        <Box sx={{ display: isXs ? 'none' : 'flex' }} className={classes.header2}>
          <ImageSlide2 />
        </Box>
      ...
      <Box className={classes.connectDiv}>
        <Box className={classes.walletConnectContent}>
          ...
          {(steps == 2 || steps == 3) && mintablenfts.length == 0 ? (
            ...
            <>
              ...
              {wallet != null ? (
                wallet && wallet.connected && !wallet.disconnecting ? (
                  <BelowButton />
                ) : (
